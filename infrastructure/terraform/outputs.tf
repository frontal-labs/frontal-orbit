...
output "ecr_repositories" {
  ...
  value = {
    orbit_server = var.deploy_orbit_server && var.orbit_server_image != "orbit-server:latest" ? aws_ecr_repository.orbit_server[0].repository_url : null
    orbit_slack = var.deploy_orbit_slack && var.orbit_slack_image != "orbit-slack:latest" ? aws_ecr_repository.orbit_slack[0].repository_url : null
  }
}
