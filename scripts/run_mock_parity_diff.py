...
def run_harness(workspace_root: Path) -> dict:
...
def main() -> int:
    ...
    manifest = load_manifest(workspace_root / "mock_parity_scenarios.json")
    parity_text = load_parity_text(workspace_root / "PARITY.md")
    ...
    report = run_harness(workspace_root) if should_run else None
    report_by_name = {
    ...
    print(f"Scenario manifest: {workspace_root / 'mock_parity_scenarios.json'}")
    print(f"PARITY source: {workspace_root / 'PARITY.md'}")
    print()
