...
# Orbitable Coding Harness Roadmap

## Goal

Turn orbit into the most **orbitable** coding harness:
...
This roadmap assumes the primary users are **orbits wired through hooks, plugins, sessions, and channel events**.

## Definition of "orbitable"

An orbitable harness is:
...
- tmux state
- orbithip event stream
- git/worktree state
- test state
- gateway/plugin/MCP runtime state

### 3. Events are too log-shaped
- orbits currently infer too much from noisy text
...
### 7. Human UX still leaks into orbit workflows
...
Acceptance:
- an orbit can operate a coding worker without raw send-keys as the primary control plane

## Phase 2 — Event-Native Orbithip Integration
...
Acceptance:
- orbithip consumes typed lane events
...
- orbits stop inferring state from raw build spam
...
## Phase 4 — Orbits-First Task Execution
...
Acceptance:
- orbits can dispatch work without relying on long natural-language prompt blobs alone
...
### 12. Orbit-native dashboards / lane board
...
Acceptance:
- orbits can query status directly
...
Priority order: P0 = blocks CI/green state, P1 = blocks integration wiring, P2 = orbitability hardening, P3 = swarm-efficiency improvements.

**P0 — Fix first (CI reliability)**
1. Isolate `render_diff_report` tests into tmpdir — **done**: `render_diff_report_for()` tests run in temp git repos instead of the live working tree, and targeted `cargo test -p orbit-cli render_diff_report -- --nocapture` now stays green during branch/worktree activity
...
5. Surface `doctor` / preflight diagnostics in onboarding docs and help — **done**: README + USAGE now put `orbit doctor` / `/doctor` in the first-run path and point at the built-in preflight report
...
7. Eliminate warning spam from first-run help/build path — **done**: current `cargo run -q -p orbit-cli -- --help` renders clean help output without a warning wall before the product surface
8. Promote `doctor` from slash-only to top-level CLI entrypoint — **done**: `orbit doctor` is now a local shell entrypoint with regression coverage for direct help and health-report output
9. Make machine-readable status commands actually machine-readable — **done**: `orbit --output-format json status` and `orbit --output-format json sandbox` now emit structured JSON snapshots instead of prose tables
10. Unify legacy config/skill namespaces in user-facing output — **done**: skills/help JSON/text output now present `.orbit` as the canonical namespace and collapse legacy roots behind `.orbit`-shaped source ids/labels
...
**P2 — Orbitability hardening (original backlog)**
...
7. Canonical lane event schema in orbithip — **done**: `LaneEvent` enum with `Started/Blocked/Failed/Finished` variants, `LaneEvent::new()` typed constructor, `tools/src/lib.rs` integration
...
17. **Orphaned module integration audit** — **done**: `runtime` now keeps `session_control` and `trust_resolver` behind `#[cfg(test)]` until they are wired into a real non-test execution path, so normal builds no longer advertise dead orbitability surface area.
...
19. **Subcommand help falls through into runtime/API path** — **done**: `orbit doctor --help`, `orbit status --help`, `orbit sandbox --help`, and nested `mcp`/`skills` help are now intercepted locally without runtime/provider startup, with regression tests covering the direct CLI paths.
...
26. **Resumed local-command JSON parity gap** — **done**: direct `orbit --output-format json` already had structured renderers for `sandbox`, `mcp`, `skills`, `version`, and `init`, but resumed `orbit --output-format json --resume <session> /…` paths still fell back to prose because resumed slash dispatch only emitted JSON for `/status`. Resumed `/sandbox`, `/mcp`, `/skills`, `/version`, and `/init` now reuse the same JSON envelopes as their direct CLI counterparts, with regression coverage in `crates/orbit-cli/tests/resume_slash_commands.rs` and `crates/orbit-cli/tests/output_format_contract.rs`.
**P3 — Swarm efficiency**
...
### Session B — orbithip lane events
Focus:
...
We should consider orbit materially more orbitable when:
- an orbit can start a worker and know with certainty when it is ready
- orbits no longer accidentally type tasks into the shell
- stale-branch failures are identified before they waste debugging time
- orbithip reports machine states, not just tmux prose
...
orbit should evolve from:
- a CLI a human can also drive

to:
- an **orbit-native execution runtime**
