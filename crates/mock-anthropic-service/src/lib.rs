...
use orbit_api::{InputContentBlock, MessageRequest, MessageResponse, OutputContentBlock, Usage};
...
fn flatten_tool_result_content(content: &[orbit_api::ToolResultContentBlock]) -> String {
    content
        .iter()
        .map(|block| match block {
            orbit_api::ToolResultContentBlock::Text { text } => text.clone(),
            orbit_api::ToolResultContentBlock::Json { value } => value.to_string(),
        })
        .collect::<Vec<_>>()
        .join("\n")
}
