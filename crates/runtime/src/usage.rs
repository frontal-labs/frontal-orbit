...
// Sonnet-tier list pricing, in US dollars per million tokens. Cache writes are
// 1.25x the input rate and cache reads 0.1x, per the published cache pricing.
...
pub fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    if normalized.contains("opus") {
        // Opus has been $5/$25 per million since the 4.x generation; cache
        // writes are 1.25x input and cache reads 0.1x input.
        return Some(ModelPricing {
