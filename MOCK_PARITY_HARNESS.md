...
This milestone adds a deterministic Anthropic-compatible mock service plus a reproducible CLI harness for the Rust `orbit` binary.

## Artifacts

- `crates/mock-anthropic-service/` — mock `/v1/messages` service
- `crates/orbit-cli/tests/mock_parity_harness.rs` — end-to-end clean-environment harness
